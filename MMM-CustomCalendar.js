...
Module.register("MMM-CustomCalendar", {
	...
	defaults: {
		...
		// --- MMM-CustomCalendar additions: instance-level formatting defaults. ---
		// Each of these can also be set per-calendar inside the `calendars` array
		// entry to override the instance-level value for just that calendar.
		// Any value left as null falls back to the module's stylesheet default
		// (i.e. behaves exactly like the stock calendar module).
		fontSize: null, // e.g. "28px" - general fallback size for rows not covered below
		titleFontSize: null, // e.g. "28px"
		timeFontSize: null, // e.g. "24px"
		symbolFontSize: null, // e.g. "30px"
		lineHeight: null, // e.g. "1.25"
		symbolTitleSpace: null, // e.g. "25px" - space between the symbol and the title
		titleDateSpace: null, // e.g. "30px" - space between the title and the date/time
	...
	getDom () {
		...
		events.forEach((event, index) => {
			...
			this.setStyleIfDefined(eventWrapper, "fontSize", this.styleForUrl(event.url, "fontSize"));
			this.setStyleIfDefined(eventWrapper, "lineHeight", this.styleForUrl(event.url, "lineHeight"));
			...
			if (this.config.displaySymbol) {
				...
				this.setStyleIfDefined(symbolWrapper, "fontSize", this.styleForUrl(event.url, "symbolFontSize"));
				this.setStyleIfDefined(symbolWrapper, "paddingRight", this.styleForUrl(event.url, "symbolTitleSpace"));
			...
			const resolvedMaxTitleLength = this.getCalendarProperty(event.url, "maxTitleLength", this.config.maxTitleLength);
			titleWrapper.innerHTML = CalendarUtils.shorten(transformedTitle, resolvedMaxTitleLength, this.config.wrapEvents, this.config.maxTitleLines) + repeatingCountTitle;
			...
			this.setStyleIfDefined(titleWrapper, "fontSize", this.styleForUrl(event.url, "titleFontSize"));
			...
			if (this.config.timeFormat === "dateheaders") {
				...
				this.setStyleIfDefined(timeWrapper, "fontSize", this.styleForUrl(event.url, "timeFontSize"));
				this.setStyleIfDefined(timeWrapper, "paddingLeft", this.styleForUrl(event.url, "titleDateSpace"));
			...
			if (this.config.showLocation) {
				if (event.location !== false) {
					...
					this.setStyleIfDefined(locationRow, "fontSize", this.styleForUrl(event.url, "fontSize"));
					this.setStyleIfDefined(locationRow, "lineHeight", this.styleForUrl(event.url, "lineHeight"));
	...
	createDateHeadersTimeWrapper (url) {
		...
		this.setStyleIfDefined(timeWrapper, "fontSize", this.styleForUrl(url, "timeFontSize"));
		this.setStyleIfDefined(timeWrapper, "paddingLeft", this.styleForUrl(url, "titleDateSpace"));
		return timeWrapper;
	},
	...
	/**
	 * MMM-CustomCalendar addition.
	 * Applies a resolved formatting value to an element's inline style, but only
	 * if a value was actually configured (per-calendar or instance-level). This
	 * keeps the stock stylesheet's own defaults intact when nothing is configured.
	 * @param {HTMLElement} element The element to style
	 * @param {string} cssProperty The camelCase CSS property name (e.g. "fontSize")
	 * @param {string} value The resolved value, or null/undefined if unset
	 */
	setStyleIfDefined (element, cssProperty, value) {
